...
class NissanConnectNASessionWrapper extends LeafSessionInternal {
  ...
  Future<void> login() async {
    ...
    final List<VehicleInternal> newVehicles = _session.vehicles
        .map((NissanConnectVehicle vehicle) => NissanConnectNAVehicleWrapper(vehicle))
        .toList();
...
class NissanConnectNAVehicleWrapper extends VehicleInternal {
  ...
  Future<Map<String, String>> fetchDailyStatistics(DateTime targetDate) async =>
      fetchStatistics(
          TimeRange.Daily, await _getVehicle().requestDailyStatistics(targetDate));

  @override
  Future<Map<String, String>> fetchMonthlyStatistics(DateTime targetDate) async =>
      fetchStatistics(
          TimeRange.Monthly, await _getVehicle().requestMonthlyStatistics(targetDate));
  ...
  @override
  Future<Map<String, String>> fetchClimateStatus() => Future.value(
        saveAndPrependVin(
          ClimateInfoBuilder()
